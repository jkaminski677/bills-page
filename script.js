...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const dataForm = document.getElementById("data-form-div");
    const addNewCategoryButton = document.getElementById("add-new-category");
    const categoryForm = document.getElementById("category-form-div");
    const addCategoryButton = document.getElementById("add-category-button");
    const addValueButton = document.getElementById("add-value-button");
    const endingButtons = document.getElementById("ending-buttons-div");
    const theEndButton = document.getElementById("the-end");
    const showTablesButton = document.getElementById("show-tables");
    const tables = document.getElementById("tabels");
    const exitButton = document.getElementById("exit-button");

    // Funkcja do ustawienia animacji i zarządzania widocznością
    const animateElement = (element, animation, callback) => {
        element.style.animation = `${animation} 0.5s forwards`;
        element.addEventListener("animationend", () => {
            if (callback) callback();
            element.style.animation = ""; // Reset animacji
        }, { once: true });
    };

    // Ukrywanie wszystkich elementów oprócz początkowego
    const hideAllExcept = (visibleElement) => {
        const allElements = [addRowButton, dataForm, categoryForm, endingButtons, tables];
        allElements.forEach(el => {
            if (el !== visibleElement) el.style.display = "none";
        });
        if (visibleElement) visibleElement.style.display = "block";
    };

    // Początkowe ukrycie wszystkich elementów poza addRowButton
    hideAllExcept(addRowButton);

    // Obsługa zdarzeń
    addRowButton.addEventListener("click", () => {
        animateElement(addRowButton, "slideOutUp", () => {
            hideAllExcept(dataForm);
            animateElement(dataForm, "slideInUp");
        });
    });

    addNewCategoryButton.addEventListener("click", () => {
        animateElement(dataForm, "slideOutRight", () => {
            hideAllExcept(categoryForm);
            animateElement(categoryForm, "slideInLeft");
        });
    });

    addCategoryButton.addEventListener("click", () => {
        animateElement(categoryForm, "slideOutLeft", () => {
            hideAllExcept(dataForm);
            animateElement(dataForm, "slideInRight");
        });
    });

    addValueButton.addEventListener("click", (e) => {
        e.preventDefault();
        animateElement(dataForm, "slideOutUp", () => {
            hideAllExcept(endingButtons);
            animateElement(endingButtons, "slideInUp");
        });
    });

    theEndButton.addEventListener("click", () => {
        animateElement(endingButtons, "slideOutUp", () => {
            hideAllExcept(addRowButton);
            animateElement(addRowButton, "slideInUp");
        });
    });

    showTablesButton.addEventListener("click", () => {
        animateElement(endingButtons, "slideOutUp", () => {
            hideAllExcept(tables);
            animateElement(tables, "slideInUp");
        });
    });

    exitButton.addEventListener("click", () => {
        const visibleElements = [dataForm, categoryForm, endingButtons, tables];
        visibleElements.forEach(el => {
            if (el.style.display === "block") {
                animateElement(el, "slideOutUp", () => {
                    hideAllExcept(addRowButton);
                    animateElement(addRowButton, "slideInUp");
                });
            }
